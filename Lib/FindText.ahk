...
Lang(text:="", getLang:=0)
{
  ...
  if (!Lang1)
  {
    s:="
    (
Myww = 너비 = 캡처 범위의 너비 조정
Myhh = 높이 = 캡처 범위의 높이 조정
AddFunc = FindText() 첨부 = 복사 시 FindText() 함수 포함
NowHotkey = 캡처 단축키 = 현재 설정된 캡처 단축키
SetHotkey1 = = 1순위 캡처 단축키
SetHotkey2 = = 2순위 캡처 단축키
Apply = 적용 = 새로운 캡처 단축키 적용
CutU2 = 상단자르기 = 텍스트 상단 자르기
CutL2 = 좌측자르기 = 텍스트 좌측 자르기
CutR2 = 우측자르기 = 텍스트 우측 자르기
CutD2 = 하단자르기 = 텍스트 하단 자르기
Update = 업데이트 = 텍스트를 코드에 업데이트
GetRange = 화면범위가져오기 = 화면 범위를 클립보드로 복사하고 코드 교체
GetOffset = 상대좌표얻기 = 이미지 상대 좌표 계산 후 클릭 좌표 교체
GetClipOffset = 상대좌표얻기2 = 왼쪽 텍스트 상자 기준 상대 좌표 계산
Capture = 캡처 = 화면 캡처 시작
CaptureS = 스크린샷캡처 = 전체 캡처 후 수동으로 캡처 범위 지정
Test = 테스트 = 코드가 이미지를 찾는지 테스트
TestClip = 테스트2 = 텍스트 데이터 테스트 후 결과를 클립보드로 복사
Paste = 붙여넣기 = 클립보드 텍스트 붙여넣기
CopyOffset = 복사2 = 상대 좌표를 클립보드로 복사
Copy = 복사 = 코드를 클립보드로 복사
Reset = 초기화 = 원본 이미지 다시 불러오기
SplitAdd = 분할추가 = 이미지를 분할하여 코드에 추가
AllAdd = 전체추가 = 텍스트 데이터 전체 추가
Gray2Two = 회색임계값 = 임계값보다 낮으면 흑, 아니면 백
GrayDiff2Two = 회색차이값 = 주변 픽셀 차이가 크면 흑, 아니면 백
Color2Two = 색상임계값 = 지정 색상 기반 흑백 변환
ColorPos2Two = 색상위치 = 지정 색상은 흑, 위치 기록
SelGray = 회색 = 회색 값 선택 (0-255)
SelColor = 색상 = RGB 색상 선택
SelR = Red = 빨간색 (R) 값 선택
SelG = Green = 녹색 (G) 값 선택
SelB = Blue = 파란색 (B) 값 선택
RepU = -상단 = 상단 자르기 취소 1px
CutU = 상단 = 상단 자르기 1px
CutU3 = 상단3 = 상단 자르기 3px
RepL = -좌측 = 좌측 자르기 취소 1px
CutL = 좌측 = 좌측 자르기 1px
CutL3 = 좌측3 = 좌측 자르기 3px
Auto = 자동 = 흑백 변환 후 여백 자동 자르기
RepR = -우측 = 우측 자르기 취소 1px
CutR = 우측 = 우측 자르기 1px
CutR3 = 우측3 = 우측 자르기 3px
RepD = -하단 = 하단 자르기 취소 1px
CutD = 하단 = 하단 자르기 1px
CutD3 = 하단3 = 하단 자르기 3px
Modify = 수정 = 마우스 클릭으로 흑백 픽셀 수동 수정
MultiColor = 다중색상 = 여러 색상 선택 후 확인
FindShape = 모양찾기 = 여러 색상 선택 (첫번째 색상 기준)
Undo = 취소 = 마지막 색상 선택 취소
Undo2 = 취소 = 목록에서 마지막 추가 색상 취소
Comment = 문자식별 = 문자로 분리 식별
Threshold = 임계값 = 회색 임계값 (0-255)
GrayDiff = 회색차이 = 회색 차이값 (0-255)
Similar1 = 유사도 = 색상 유사도
Similar2 = 유사도 = 색상 유사도
Similar3 = 유사도 = 색상 유사도
AddColorSim = 색상추가 = 유사 색상 목록 추가 후 변환
AddColorDiff = 색상추가 = 편차 색상 목록 추가 후 변환
ColorList = = 흑백 변환을 위한 색상 목록
DiffRGB = R/G/B = 편차 (0-255)
DiffRGB2 = R/G/B = 편차 (0-255)
Bind0 = 창바인딩1 = GetDCEx() 백그라운드 캡처
Bind1 = 창바인딩1+ = GetDCEx() 투명도 조정 캡처
Bind2 = 창바인딩2 = PrintWindow() 백그라운드 캡처
Bind3 = 창바인딩2+ = PrintWindow() 투명도 조정 캡처
Bind4 = 창바인딩3 = PrintWindow(,,3) 백그라운드 캡처
OK = 확인 = 코드 생성 및 교체
OK2 = 확인 = 스크린샷 복원 후 캡처
Cancel = 취소 = 닫기
Cancel2 = 취소 = 닫기
ClearAll = 모두지우기 = 저장된 모든 캡처 삭제
OpenDir = 폴더열기 = 캡처 저장 폴더 열기
SavePic = 이미지저장 = 범위 지정 후 저장
SavePic2 = 이미지저장 = 원본 이미지를 이미지로 저장
LoadPic = 이미지불러오기 = 캡처할 이미지 로드
ClipText = = 텍스트 데이터 확인
Offset = = 상대좌표 또는 범위 결과
SelectBox = = 스크린샷 좌상단에 표시
s1 = FindText 캡처 및 이미지 인식 도구
s2 = 회색임계값|회색차이|색상|색상위치|다중색상
s3 = 이미지 흑백 변환 및 분할
s4 = 화면 캡처 및 코드 생성기
s5 = 방향키로 범위 미세조정\n오른쪽 마우스(Ctrl) 1번 클릭\n마우스 이동 후\n오른쪽 마우스(Ctrl) 다시 클릭
s6 = 창 바인딩 해제
s7 = 왼쪽 마우스(Ctrl)로 드래그\n결과가 클립보드로 복사됨
s8 = 발견됨|시간|ms|위치|결과|값얻기|3초동안기다리기|무한대기
s9 = 캡처 성공
s10 = 마우스위치|백그라운드 창 무시 캡처\n우클릭으로 완료
s11 = 먼저 회색 차이값을 설정하세요!
s12 = 먼저 기준 색상을 선택하세요!
s13 = 먼저 이미지를 흑백으로 변환하세요!
s14 = 위치 기록 모드에서는 지원하지 않습니다
s15 = 이 범위가 맞습니까?\n\n아니면 다시 선택할 수 있습니다.
s16 = 먼저 색상 목록에 추가하세요!
s17 = 이미지를 찾을 수 없습니다!
s18 = 캡처|스크린샷
